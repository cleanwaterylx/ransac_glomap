...
def recursive_NCut(W, idx2name, max_group_size=10, min_group_size=3, depth=0):
    """
    ...
    clusters = []
    all_too_small = True  # 检查是否所有子簇都太小
    ...
    for label in np.unique(labels):
        ...
        if len(indices) < min_group_size:
            ...
            return [list(idx2name.values())]  # 放弃这次分割，保留原簇
        else:
            all_too_small = False
...
def NCut(W): 
    ...
    sc = SpectralClustering(n_clusters=6, affinity='precomputed', random_state=0) 
    labels = sc.fit_predict(W) 
    clusters = defaultdict(list) 
    for idx, label in zip(range(W.shape[0]), labels): 
        print(f"{idx2name[idx]}: cluster {label}") 
        img_name = idx2name[idx] 
...
if __name__ == '__main__':
    ...
    input()
    W = parse_image_pair_file(f'{name}/sparse/image_pair_inliers_relpose.txt', imgs)
    print(name2idx)
    print(idx2name)
